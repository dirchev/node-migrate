...
function migrate (set, direction, migrationName, fn) {
  ...
  function next (migration) {
    ...
    if (direction === 'up') {
      var returnValue = migration[direction](function (err, result) {
        if (isPromise) return set.emit('warning', 'if your migration returns a promise, do not call the done callback')
        completeMigration(err, result)
      })
    } else {
      var lastRunResult = migration.lastRunResult
      var returnValue = migration[direction](lastRunResult, function (err, result) {
        if (isPromise) return set.emit('warning', 'if your migration returns a promise, do not call the done callback')
        completeMigration(err, result)
      })
    }
    ...
    if (isPromise) {
      returnValue
        .then((result) => completeMigration(null, result))
        .catch(fn)
    }

    function completeMigration (err, result) {
      if (err) return fn(err)

      // Set timestamp if running up, clear it if down
      migration.timestamp = direction === 'up' ? Date.now() : null
      migration.lastRunResult =  direction === 'up' ? result : null
